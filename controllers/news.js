...
exports.topFr = async (req, res, next) => {
  ...
  // Articles params => source, author, title, description, url, urlToImage, publishedAt, content
    for (let variable in response.articles) {
      headlines.push({
...
exports.topFrBusiness = async (req, res, next) => {
  ...
  let headlines = await newsFr('business', 'fr')
...
exports.topFrEnt = async (req, res, next) => {
  ...
  let headlines = await newsFr('entertainment', 'fr')
...
exports.topFrSante = async (req, res, next) => {
  ...
  let headlines = await newsFr('health', 'fr')
...
exports.topFrScience = async (req, res, next) => {
  ...
  let headlines = await newsFr('science', 'fr')
...
exports.topFrSports = async (req, res, next) => {
  ...
  let headlines = await newsFr('sports', 'fr')
...
exports.topFrTec = async (req, res, next) => {
  ...
  let headlines = await newsFr('technology', 'fr')
...
exports.topEnBusiness = async (req, res, next) => {
  ...
  let headlines = await news('australian-financial-review,bloomberg,business-insider,cnbc,financial-post,forturne,the-wall-street-journal', 'en')
...
exports.topEnEnt = async (req, res, next) => {
  ...
  let headlines = await news('buzzfeed,entertainment-weekly,ign,mashable,mtv-news,mtv-news-uk,polygon,the-lad-bible', 'en')
...
exports.topEnHealth = async (req, res, next) => {
  ...
  let headlines = await news('medical-news-today', 'en')
...
exports.topEnScience = async (req, res, next) => {
  ...
  let headlines = await news('national-geographic,new-scientist', 'en')
...
exports.topEnSports = async (req, res, next) => {
  ...
  let headlines = await news('bbc-sport,bleacher-report,espn,espn-cric-info,football-italia,four-four-two,fox-sports,nfl-news,nhl-news,talksport,the-sport-bible', 'en')

  return res.render('index', { headlines: headlines, lang: 'en/', categories: en, connected: isConnected })
}

exports.topEnTec = async (req, res) => {
  let isConnected = false
  if (req.session.user_id) {
    isConnected = true
  }

  let headlines = await news('ars-technica,crypto-coins-news,engadget,hacker-news,recode,techcrunch,techradar,the-next-web,the-verge,wired', 'en')
...
async function news (sources, lang) {
  ...
  const res = await newsapi.v2.topHeadlines({
    ...
    for (let variable in res.articles) {
      headlines.push({
        // Articles params => source, author, title, description, url, urlToImage, publishedAt, content
        title: res.articles[variable].title,
        description: res.articles[variable].description,
        url: res.articles[variable].url,
        urlToImage: res.articles[variable].urlToImage
      })
    }
  return headlines
}
